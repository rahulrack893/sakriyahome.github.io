var wcfe_settings = (function($, window, document) {
	...
	function wcfe_add_new_row(form){
		...
		var extoptionsList = $(form).find("input[name=fextoptions]").val();
		var maxfile = $(form).find("input[name=fmaxfile]").val();
		...
		newRow += '<input type="hidden" name="f_maxfile['+index+']" class="f_maxfile" value="'+maxfile+'" />';
	...
	_openEditFieldForm = function openEditFieldForm(elm, rowId){
		...
		var maxfile = row.find(".f_maxfile").val();
	...
	function wcfe_update_row(form){
		...
		row.find(".f_maxfile").val(maxfile);
	...
	function wcfe_clear_form( form ){
		...
		form.find("input[name=fmaxfile]").val('');
	...
	_fieldTypeChangeListner = function fieldTypeChangeListner(elm){
		...
		if(type === 'select'){			
		...
		else if(type === 'password' || type === 'file' || type === 'date' || type === 'heading' || type === 'hidden' || type === 'multiselect' || type === 'radio' || type === 'checkbox' || type === 'heading'){
			form.find('.rowPremium').show();
			$('.ui-dialog-buttonpane').hide();
			form.find('.rowType').hide();
			...
			form.find('.rowPlaceholder').hide();
			form.find('.rowMaxfile').hide();
			form.find('.rowRequired').hide();
			form.find('.rowShowInEmail').hide();
			form.find('.rowShowInOrder').hide();
	...
	function showAllFields(form){
		...
		form.find('.rowShowInEmail').show();
		form.find('.rowType').show();
		form.find('.rowName').show();
		form.find('.rowShowInOrder').show();
		form.find('.rowExtoptions').hide();
		form.find('.rowMaxfile').hide();
		form.find('.rowPremium').hide();
	}
