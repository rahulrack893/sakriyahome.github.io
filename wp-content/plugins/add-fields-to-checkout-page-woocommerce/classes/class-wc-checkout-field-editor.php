...
class WC_Checkout_Field_Editor {
	...
	function __construct() {
		...
		add_filter( 'woocommerce_form_field_file', array($this, 'wcfe_checkout_fields_file_field'), 10, 4 );
	...
	function enqueue_admin_scripts() {
		...
		'woocommerce_admin', 'select2', 'jquery-tiptip'), WCFE_VERSION, true );
		    ...
		    'MSG_INVALID_NAME' => 'NAME/ID must begin with a lowercase letter ([a-z]) and may be followed by any number of lowercase letters, digits ([0-9]) and underscores ("_")'
		  ));	
	}
	...
	function wc_checkout_fields_scripts() {
		...
		if ( is_checkout() || is_account_page()) {
			...
			wp_enqueue_script( 'wcfe-upload', plugins_url('/assets/js/wcfe-upload.js', dirname(__FILE__)), array('jquery'), '1.0.0', true );
	  		wp_localize_script( 'wcfe-upload', 'MyAjax', array(
		    // URL to wp-admin/admin-ajax.php to process the request
		    'ajaxurl' => admin_url( 'admin-ajax.php' ),
		    'fileurl' => plugins_url('/includes/wcfe-file-upload.php', dirname(__FILE__))
		  ));
            	...
            	<table>
                	<tr>
                    	<td width="70%">
                            ...
                            <ul>
                            	<li><?php esc_html_e('12 field types available,','wcfe'); ?><br/>(<i><?php esc_html_e('Text, Hidden, Password, Textarea, Radio, Checkbox, Select, Multi-select, Date Picker, Time Picker, Heading, Label','wcfe'); ?></i>).</li>
			...
			if($key === 'billing' || $key === 'shipping'){
				...
				$fields = array(
					'order_comments' => array(
						...
						'placeholder' => esc_html__('Notes about your order, e.g. special notes for delivery.', 'placeholder', 'woocommerce')
					)
				);
			}
        ...
        <div id="wcfe_new_field_form_pp" title="New Checkout Field" class="wcfe_popup_wrapper">
          <form>
          	<table>
            	...
            	<tr class="rowType">                    
                    ...
                    <td>
                        ...
                        <?php foreach($field_types as $value=>$label){
                        	...
                        	if(isset($_GET['section']) && $_GET['section'] != 'account' && $value == 'file')
                        	{
                        		continue;
                        	}
            	...
            	<tr class="rowName">                
                	<td><?php esc_html_e('Name','wcfe'); ?></td>
                    <td><input type="text" name="fname" style="width:250px;"/></td>
				</tr>         
                <tr class="rowLabel">
                    <td><?php esc_html_e('Label','wcfe'); ?></td>
                ...
                <tr class="rowOptions">                    
                    <td><?php esc_html_e('Options','wcfe'); ?></td>
                    <td><input type="text" name="foptions" placeholder="Seperate options with pipe(|)" style="width:250px;"/></td>
				</tr>
                <tr class="rowClass">
                    <td><?php esc_html_e('Class','wcfe'); ?></td>
                    <td>
                    	<input type="text" name="fclass" placeholder="Seperate classes with comma" style="width:250px;"/>
                    	<br><span><?php esc_html_e('- Full Width -> form-row-wide','wcfe'); ?><br><?php esc_html_e('- Half Width -> form-row-first ; form-row-second','wcfe'); ?></span>
				...
				<tr class="rowExtoptions">                    
                    <td><?php esc_html_e('Allowed file types','wcfe'); ?></td>
                    <td><input type="text" name="fextoptions" placeholder="Seperate file ext with pipe(|)" style="width:250px;"/></td>
				</tr>

				<tr class="rowMaxfile">                    
                    <td><?php esc_html_e('Maximum filesize','wcfe'); ?></td>
                    <td><input type="number" name="fmaxfile" style="width:250px;"/></td>
				</tr>
                <tr class="rowRequired">
                    ...
                    <td>                    	
                        ...
                        <label><?php esc_html_e('Clear Row','wcfe'); ?></label><br/>
                ...
                <tr class="rowShowInOrder"> 
            	 ...
            	 <tr class="rowPremium">                    
                    <td>
                    	...
                    	<a target="_blank" href="https://www.themelocation.com/woocommerce-checkout-register-form-editor/" class="">
                            	<img src="<?php echo plugins_url( '../assets/css/update_notify.png', __FILE__ ); ?>" />
                            </a>
        ...
        <div id="wcfe_edit_field_form_pp" title="Edit Checkout Field" class="wcfe_popup_wrapper">
          <form>
          	<table>
            	...
            	<tr>                
                	<td width="40%"><?php esc_html_e('Name','wcfe'); ?></td>
                    <td>
	...
	function render_checkout_fields_heading_row(){
		...
		<th class="status"><?php esc_html_e('Clear Row','wcfe'); ?></th>
	...
	function render_actions_row($section){
		?>
        <th colspan="7">
            <button type="button" class="button button-primary" onclick="openNewFieldForm('<?php echo $section; ?>')"><?php _e( '+ Add field', 'wcfe' ); ?></button>
					...
					?>
						...
						<tr class="row_<?php echo $i; echo($options['enabled'] == 1 ? '' : ' wcfe-disabled') ?>">
                        	...
                        	<td width="1%" class="sort ui-sortable-handle">
                                ...
                                <input type="hidden" name="f_maxfile[<?php echo $i; ?>]" class="f_maxfile" value="<?php echo $options['maxfile']; ?>" />
                                ...
                                <input type="hidden" name="f_extoptions[<?php echo $i; ?>]" class="f_extoptions" value="<?php echo($options['options']) ?>" />
	...
	function save_options( $section ) {
		...
		$f_maxfiles      = ! empty( $_POST['f_maxfile'] ) ? $_POST['f_maxfile'] : array();
		...
		$f_extvalidation  = ! empty( $_POST['f_validation'] ) ? $_POST['f_validation'] : array();
		...
		for ( $i = 0; $i <= $max; $i ++ ) {
			...
			$fields[$name]['maxfile']   	  = empty( $f_maxfiles[$i] ) ? '' : wp_kses_post( trim( stripslashes( $f_maxfiles[$i] ) ) );
			...
			$fields[$name]['extoptions'] 	  = empty( $f_extoptions[$i] ) ? array() : array_map( 'wc_clean', explode( '|', trim(stripslashes($f_extoptions[$i])) ) );
			...
			if (!empty( $fields[$name]['extoptions'] )) {
				$fields[$name]['extoptions'] = array_combine( $fields[$name]['extoptions'], $fields[$name]['extoptions'] );
			}
			...
			$fields[$name]['label']   	  = esc_html__($fields[$name]['label'], 'wcfe');
			$fields[$name]['placeholder'] = esc_html__($fields[$name]['placeholder'], 'wcfe');

			$fields[$name]['maxfile'] = __($fields[$name]['maxfile'], 'woocommerce');
