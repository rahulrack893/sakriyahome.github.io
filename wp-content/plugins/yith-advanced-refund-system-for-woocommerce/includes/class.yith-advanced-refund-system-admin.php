...
if ( ! class_exists( 'YITH_Advanced_Refund_System_Admin' ) ) {
    ...
    class YITH_Advanced_Refund_System_Admin {
        ...
        public function enqueue_scripts( $hook_suffix ) {
	        ...
	        if ( 'yith_refund_request' == $current_screen->id || 'product' == $current_screen->id ) {
		        wp_register_script(
			        'ywcars-admin',
                ...
                <ul class="order_actions submitbox">
                    <li class="wide">
                        ...
                        foreach ( $requests as $request_id ) {
	                        ...
	                        $finished_request = apply_filters( 'ywcars_open_request', ( 'ywcars-approved' == $request->status || 'ywcars-rejected' == $request->status || 'trash' != $request->status ), $request );
                            ...
                            <div class="ywcars_action_list_element">
                                <input type="checkbox" name="ywcars_selected_requests[]"
                                       value="<?php echo $request_id; ?>" <?php echo $finished_request ? $title_if_finished_request : ''; ?>>
                    ...
                    if ( $requests ) {
	                    foreach ( $requests as $request_id ) {
		                    ...
		                    if ( $request ) : ?>
			                    ...
			                    $src = YITH_WCARS_ASSETS_URL . 'images/' . $request->status . '.png';
			                    $request_link = admin_url( 'post.php?post=' . absint( $request_id ) . '&action=edit' );
			                    $link_title = $request->whole_order ? __( 'Request for entire order', 'yith-advanced-refund-system-for-woocommerce' ) : wc_get_product( $request->product_id )->get_formatted_name();
